...
class TestGatingService:
    @pytest.mark.asyncio
    async def test_select_tools_by_ids(
        self, gating_service, mock_tool_repo, sample_tools
    ):
        # Arrange
        ...
        # Act
        result = await gating_service.select_tools(tool_ids=selected_ids, max_tools=5)
    ...
    @pytest.mark.asyncio
    async def test_select_tools_with_max_limit(
        self, gating_service, mock_tool_repo, sample_tools
    ):
        # Arrange
        ...
        result = await gating_service.select_tools(
            tool_ids=[t.id for t in sample_tools], max_tools=3
        )
    ...
    @pytest.mark.asyncio
    async def test_select_tools_with_token_budget(
        self, gating_service, mock_tool_repo, sample_tools
    ):
        # Arrange
        mock_tool_repo.get_by_ids.return_value = sample_tools
        gating_service.max_tokens = (
            250  # Can fit tool-1 (50) + tool-2 (100) + tool-5 (75) = 225
        )

        # Act
        result = await gating_service.select_tools(
            tool_ids=[t.id for t in sample_tools], max_tools=10
        )
    ...
    @pytest.mark.asyncio
    async def test_select_tools_no_ids_uses_popular(
        self, gating_service, mock_tool_repo, sample_tools
    ):
        # Arrange
    ...
    async def test_format_for_mcp(self, gating_service, sample_tools):
        ...
        assert mcp_tools[0].inputSchema == {
            "type": "object"
        }  # Default schema when no params
    ...
    async def test_empty_tool_selection(self, gating_service, mock_tool_repo):
        ...
        # Act
        result = await gating_service.select_tools(
            tool_ids=["non-existent"], max_tools=5
        )
    ...
    async def test_token_budget_enforcement(self, gating_service, mock_tool_repo):
        ...
        tools = [
            Tool(
                id="small", name="Small", description="Small tool", estimated_tokens=50
            ),
            Tool(
                id="medium",
                name="Medium",
            ...
            Tool(
                id="large", name="Large", description="Large tool", estimated_tokens=300
            ),
        ...
        # Act
        result = await gating_service.select_tools(
            tool_ids=[t.id for t in tools], max_tools=10
        )
    ...
    @pytest.mark.asyncio
    async def test_user_context_handling(
        self, gating_service, mock_tool_repo, sample_tools
    ):
        # Arrange
        mock_tool_repo.get_by_ids.return_value = sample_tools
